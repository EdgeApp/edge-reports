...
async function main (swapFuncParams: SwapFuncParams) {
  ...
  const rSaf = await doSafello(swapFuncParams)
...
async function report (argv: Array<any>) {
  ...
  if (!doSummary) {
    ...
    const saResults = await doSummaryFunction(doSafello)
    ...
    combineResults(results, saResults)
    ...
    console.log('\n***** Safello Monthly *****')
    printTxDataMap('SAF', saResults.monthly)
    console.log('\n***** Safello Daily *****')
    printTxDataMap('SAF', saResults.daily)
    console.log('\n***** Swap Totals Monthly*****')
