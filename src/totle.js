  ...
  while (1 && !swapFuncParams.useCache) {
    try {
      ...
      for (const event of events) {
        const {id, feeAccount, totalEthTraded, totalFee} = event.returnValues
        ...
        const logs = receipt.logs.filter(({ topics: [ topic ]}) => topic === LOG_TRADE_ABI.signature)
        for (const log of logs) {
          const { isSell, token: tokenAddress, tokenAmount, ethAmount } = web3.eth.abi.decodeLog(LOG_TRADE_ABI.inputs, log.data, log.topics)
          ...
          const ssTx: ShapeShiftTx = {
            ...
            outputAmount  : isSell ? ethAmount.toString() : tokenAmount.toString(),
            timestamp
          }
      console.log(ssTx)
